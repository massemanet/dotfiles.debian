...
do systemctl list-units | grep -q "$u.service" &&
        systemctl is-enabled "$u" &&
        sudo systemctl disable --now "$u"
done
sudo apt --autoremove purge -y \
     ifupdown dhcpcd dhcpcd5 isc-dhcp-client isc-dhcp-common rsyslog
...
sudo apt-mark hold \
     avahi-daemon dhcpcd dhcpcd5 ifupdown isc-dhcp-client isc-dhcp-common \
     libnss-mdns openresolv raspberrypi-net-mods rsyslog
...
do I="/etc/wpa_supplicant/wpa_supplicant-$i.conf"
   sudo tee "$I" <<HERE
country=GI
